using Blurt.Core;
using Blurt.Dictionary;
using Shouldly;
using Xunit;

namespace Blurt.CoreTests;
...
{
    private readonly string _path = Path.Combine(Path.GetTempPath(), $"blurt-dict-{Guid.NewGuid():N}.txt");
...
{
    private readonly string _path = Path.Combine(Path.GetTempPath(), $"blurt-hist-{Guid.NewGuid():N}.jsonl");
...
{
    private readonly string _path = Path.Combine(Path.GetTempPath(), $"blurt-set-{Guid.NewGuid():N}.json");
